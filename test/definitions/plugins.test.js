...
test('The "prepare" plugin, if defined, must be a single or an array of plugins definition', t => {
  t.false(plugins.prepare.configValidator({}));
  t.false(plugins.prepare.configValidator({path: null}));

  t.true(plugins.prepare.configValidator({path: 'plugin-path.js'}));
  t.true(plugins.prepare.configValidator());
  t.true(plugins.prepare.configValidator('plugin-path.js'));
  t.true(plugins.prepare.configValidator(() => {}));
  t.true(plugins.prepare.configValidator([{path: 'plugin-path.js'}, 'plugin-path.js', () => {}]));
});
